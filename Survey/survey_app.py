...
import time
import os
os.environ['OPENAI_API_KEY'] = st.secrets['OPENAI_API_KEY'] # Streamlit community cloud에서 키 관리
...
st.write("이 페이지에서 ai와 대화해보신 후, 설문지([바로가기 링크](https://forms.gle/PgWmcqNeYEoKeFk79))를 작성해주시면 저희 프로젝트에 큰 도움이 됩니다.")
st.write("설문지에 응답하신 정보는 프로젝트 이외의 목적으로 활용되지 않으며, ai와의 대화 내용은 수집되지 않습니다.")
st.write("저희의 페이지에 들러 소중한 시간을 할애해 주셔서 감사합니다.(✿´꒳`)ﾉ°")

st.info("페이지 사용 Tip: 좌측 상단(\">\"버튼)의 사이드바에서 원하는 상담사 유형을 선택해 주세요. 아래에 질문 내용을 입력한 후, Enter 버튼을 누르는 것으로 대화를 진행할 수 있습니다. 상담사를 바꾸면 처음부터 다시 대화가 시작됩니다.")

if 'currModel' not in st.session_state:
    st.session_state['currModel'] = ""
...
if model_type == "인지 치료 상담사":
    ...
    if st.session_state['currModel'] != "인지 치료 상담사":
        st.session_state['currModel'] = "인지 치료 상담사"
...
if model_type == "WDEP 상담사":
    # 선택한 모델을 바꾸면 대화내역 초기화
    if st.session_state['currModel'] != "WDEP 상담사":
        st.session_state['currModel'] = "WDEP 상담사"
        if 'messages' in st.session_state:
            del st.session_state["messages"]
        # System message 변경
...
if model_type == "상냥한 친구":
    # 선택한 모델을 바꾸면 대화내역 초기화
    if st.session_state['currModel'] != "상냥한 친구":
        st.session_state['currModel'] = "상냥한 친구"
        if 'messages' in st.session_state:
            del st.session_state["messages"]
        # System message 변경
...
if model_type == "시니컬한 상담사":
    ...
    if st.session_state['currModel'] != "시니컬한 상담사":
        st.session_state['currModel'] = "시니컬한 상담사"
        if 'messages' in st.session_state:
            del st.session_state["messages"]
        # System message 변경
...
for message in st.session_state['messages']:
    if message['role'] == 'user':
        ...
        st.write(f"상담사: {message['content']}\n\n")

element_ask = st.empty()
element_ans = st.empty()
st.markdown("---")
...
if st.button("Enter"):
    if user_input:
        try:
            ...
            # API 응답 출력
            element_ask.markdown("당신: " + user_input)
            _LOREM_IPSUM = "상담사: " + completion
            element_ans.write_stream(stream_data)
